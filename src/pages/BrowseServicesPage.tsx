...
import { Search, ArrowUpDown, IndianRupee, Clock } from 'lucide-react';
...
export default function BrowseServicesPage() {
  ...
  // Extract search term from URL params if exists
  ...
  // Update search params when filter/search changes
  useEffect(() => {
    if (searchQuery) {
  ...
  useEffect(() => {
    ...
    // If the search query changes from outside (like navbar), update our state
    if (initialSearchQuery !== searchQuery) {
      setSearchQuery(initialSearchQuery);
    }
  }, [selectedCategory, sortBy, priceRange, deliveryTime, initialSearchQuery, location.search]);
  ...
  const fetchServices = async () => {
    ...
    try {
      ...
      } else if (priceRange === '1000_to_5000') {
        query = query.gte('price', 1000).lt('price', 5000);
      } else if (priceRange === '5000_to_10000') {
        query = query.gte('price', 5000).lt('price', 10000);
      } else if (priceRange === 'over_10000') {
      ...
      const filteredServices = initialSearchQuery 
        ? transformedServices.filter(service => 
            service.title.toLowerCase().includes(initialSearchQuery.toLowerCase()) ||
            service.description.toLowerCase().includes(initialSearchQuery.toLowerCase())
          )
        : transformedServices;
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    // We don't need to call fetchServices here since the useEffect will trigger
    // when the searchQuery or location.search changes
  };
                  ...
                  <div>
                    ...
                    <Select value={priceRange} onValueChange={setPriceRange}>
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="1000_to_5000">₹1,000 to ₹5,000</SelectItem>
                        <SelectItem value="5000_to_10000">₹5,000 to ₹10,000</SelectItem>
